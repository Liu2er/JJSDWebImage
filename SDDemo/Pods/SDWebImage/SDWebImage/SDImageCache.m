...
// 调用NSCache的setObject:forKey:cost:存储时，会同时在self.weakCache（NSMapTable strong-weak类型）里也存储一份儿
...
// 自定义缓存查询路径，即由addReadOnlyCachePath:方法添加的路径
...
- (nonnull instancetype)initWithNamespace:(nonnull NSString *)ns {
    // path：/Library/Caches/<ns>/，默认是/Library/Caches/default/
...
- (nonnull instancetype)initWithNamespace:(nonnull NSString *)ns
    ...
    if ((self = [super init])) {
        ...
        // Init the disk cache
        if (directory != nil) {
            // directory：/Library/Caches/<ns>/，默认是/Library/Caches/default/
            // _diskCachePath：/Library/Caches/<ns>/com.hackemist.SDWebImageCache.<ns>
            _diskCachePath = [directory stringByAppendingPathComponent:fullNamespace];
        } else {
            NSString *path = [self makeDiskCachePath:ns];
            // _diskCachePath：/Library/Caches/<ns>/
            _diskCachePath = path;
        }
...
- (nullable NSString *)defaultCachePathForKey:(nullable NSString *)key {
    // 例如：/Library/Caches/default/com.hackemist.SDWebImageCache.default/3320695b4e5db2b88ec71688b1f92bb7.jpg
...
- (nullable NSString *)cachedFileNameForKey:(nullable NSString *)key {
    const char *str = key.UTF8String; // key是url字符串
    if (str == NULL) {
        str = "";
    }
    unsigned char r[CC_MD5_DIGEST_LENGTH]; // CC_MD5_DIGEST_LENGTH是摘要长度，16，单位是字节
    CC_MD5(str, (CC_LONG)strlen(str), r); // CC_LONG是unsigned int
    NSURL *keyURL = [NSURL URLWithString:key];
    // 例如：@"jpg"
    NSString *ext = keyURL ? keyURL.pathExtension : key.pathExtension;
    // %x为将char转换为16进制ASCII码
    NSString *filename = [NSString stringWithFormat:@"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%@",
                          r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8], r[9], r[10],
                          r[11], r[12], r[13], r[14], r[15], ext.length == 0 ? @"" : [NSString stringWithFormat:@".%@", ext]];
    // 例如：filename = @"3320695b4e5db2b88ec71688b1f92bb7.jpg"
    return filename;
}
...
- (nullable NSString *)makeDiskCachePath:(nonnull NSString*)fullNamespace {
    ...
    // /Library/Caches/<fullNamespace>/，默认是/Library/Caches/default/
...
- (void)storeImage:(nullable UIImage *)image
    ...
    if (self.config.shouldCacheImagesInMemory) {
        ...
        // 缓存到内存的核心方法（在SDMemoryCache里被重写了）
    ...
    if (toDisk) {
        ...
        dispatch_async(self.ioQueue, ^{
            @autoreleasepool {
                ...
                // 如果image存在但imageData不存在，就由image重新编码出imageData
                if (!data && image) {
                    // If we do not have any data to detect image format, check whether it contains alpha channel to use PNG or JPEG format
                    SDImageFormat format; //枚举变量
                    // 有alpha通道就是PNG，否则就是JPEG
                ...
                // 缓存到磁盘的核心方法，即调用了NSData的writeToURL:options:error:方法
...
- (void)_storeImageDataToDisk:(nullable NSData *)imageData forKey:(nullable NSString *)key {
    ...
    // _diskCachePath默认：/Library/Caches/default/com.hackemist.SDWebImageCache.default/
    if (![self.fileManager fileExistsAtPath:_diskCachePath]) {
        [self.fileManager createDirectoryAtPath:_diskCachePath withIntermediateDirectories:YES attributes:nil error:NULL];
    }
    
    // get cache Path for image key
    // cachePathForKey例如：/Library/Caches/default/com.hackemist.SDWebImageCache.default/3320695b4e5db2b88ec71688b1f92bb7.jpg
...
- (nullable NSOperation *)queryCacheOperationForKey:(nullable NSString *)key options:(SDImageCacheOptions)options done:(nullable SDCacheQueryCompletedBlock)doneBlock {
    ...
    // First check the in-memory cache...
    UIImage *image = [self imageFromMemoryCacheForKey:key];
    // 只查询内存的情况：在内存中找到了image，且在内存中找到了图片就不再去磁盘中查找
...
- (void)deleteOldFilesWithCompletionBlock:(nullable SDWebImageNoParamsBlock)completionBlock {
    dispatch_async(self.ioQueue, ^{
        ...
        for (NSURL *fileURL in fileEnumerator) {
            ...
            //有错误，或者属性为nil，或者路径是个目录，就continue
            ...
            // 如果过期就加进要删除的集合中（laterDate:为获取较晚的时间）
